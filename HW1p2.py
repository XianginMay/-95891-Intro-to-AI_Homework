...
class GraphProblem(object):
    ...
    def h(self, node):
        ...
        if locs:
            ...
            return int(
                distance(locs[node.state], locs[self.goal]))  
        else:
            return infinity
...
if __name__ == '__main__':
    ...
    with open('p2-distances.csv', 'r') as read_obj:
        csv_reader = csv.reader(read_obj)
        for row in csv_reader:
            if row[0] not in romania_map.keys():
                romania_map[row[0]] = {row[1]:int(row[2])}
            else:
                romania_map[row[0]].update({row[1]:int(row[2])})
    ...
    romania_graph = UndirectedGraph(romania_map)
    ...
    # iterate over each line as a ordered dictionary and print only few column by column name
    with open('p2-locations.csv', 'r') as read_obj:
        csv_reader = csv.reader(read_obj)
        next(csv_reader)
        for row in csv_reader:
            romania_graph.locations[row[0]] = (int(row[1]), int(row[2]))
    # *********************************************************************
